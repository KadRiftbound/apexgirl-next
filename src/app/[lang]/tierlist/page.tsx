...
const meta: Record<string, { title: string; description: string; keywords: string }> = {
  ...
  it: { title: "TopGirl Tier List 2026 — Migliori artisti classificati", description: "Classifiche e voti per i migliori artisti di TopGirl/ApexGirl. Livelli S+, S, A, B, C, D.", keywords: "TopGirl tier list, migliori artisti TopGirl, classifica ApexGirl" },
...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical,
      languages: { fr: `${BASE_URL}/fr/tierlist/`, en: `${BASE_URL}/en/tierlist/`, it: `${BASE_URL}/it/tierlist/`, es: `${BASE_URL}/es/tierlist/`, pt: `${BASE_URL}/pt/tierlist/`, pl: `${BASE_URL}/pl/tierlist/`, id: `${BASE_URL}/id/tierlist/`, ru: `${BASE_URL}/ru/tierlist/`, "x-default": `${BASE_URL}/en/tierlist/` },
    },
...
export default async function TierlistPage({ params }: { params: Promise<{ lang: string }> }) {
  const { lang } = await params;
  return <TierlistClient lang={lang} />;
}
